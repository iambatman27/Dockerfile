...
// ---------- UI Elements ----------
const mainCatsDiv = document.querySelector('.main-cats');
...
function renderMainCategories(){
  ...
  allBtn.href = "#";
  allBtn.className = "main-btn active";
  allBtn.addEventListener("click", (e)=>{
    ...
    filterTools();
    updateMainBtnActive(allBtn);
  });
  mainCatsDiv.appendChild(allBtn);
  ...
  Object.keys(subcategories).forEach(cat=>{
    ...
    btn.addEventListener("click",(e)=>{
      ...
      renderSubcats(cat);
      filterTools();
      updateMainBtnActive(btn);
    });
    mainCatsDiv.appendChild(btn);
  });
}

function updateMainBtnActive(activeBtn){
...
function updateSubcatBtnActive(activeBtn){
...
function makeCard(tool){
  ...
  div.innerHTML = `
    ...
    <span class="badge">${escapeHtml(tool.subcategory)}</span>
...
function renderList(list){
  ...
  list.forEach(tool=>{
    cardsDiv.appendChild(makeCard(tool));
  });
}
...
function filterTools(){
  ...
  const filtered = tools.filter(t=>{
    ...
    const matchSearch = !q || (t.name+t.desc+t.category+t.subcategory).toLowerCase().includes(q);
